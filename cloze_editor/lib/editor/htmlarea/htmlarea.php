...
<?php
//Hack for CLOZE-editor: don't show the button when not teacher, and only if the parameter "cloze" is passed with value "false" via URL from weblib.php to htmlarea.php. (Query is coded in weblib.php)
if(!empty($id) and (has_capability('moodle/course:managefiles', get_context_instance(CONTEXT_COURSE, $id)) or 
			has_capability('moodle/course:managefiles', get_context_instance(CONTEXT_USER, $_GET['id'])) ) ) {

if ($_GET['cloze'] == "false"){
		$clozeedit = '';
}
else {
		$clozeedit = '"insertcloze",';
    }
}
//End hack for CLOZE-editor
?>
    ...
    this.toolbar = [
        [ "fontname", "space",
          ...
          "clean", "separator", "undo", "redo", "separator" <?php echo ",".$clozeedit; ?>  ],
    ...
    this.btnList = {
        ...
        insertcloze: [ "Insert CLOZE item", "ed_cloze_editor.png", false, function(e) {e.execCommand("insertcloze");} ],
...
/// Moodle hack - insertCLOZE; Called when the user clicks the Insert CLOZE button (based on Nadav Kavalerchiks "insertembed" hack at http://tracker.moodle.org/browse/MDL-20363)
HTMLArea.prototype._insertCloze = function() {
    var sel = "";
    if (HTMLArea.is_ie) {
		sel = this._doc.selection.createRange().text;
        var range = this._doc.selection.createRange();
	} else {
		sel = this._iframe.contentWindow.getSelection();
	}    
    var editor = this;  // for nested functions
    this._popupDialog("insert_cloze.php?id=<?php echo $id; ?>&solution="+sel, function(param) {
        var doc = editor._doc;
        // create the DIV element  to hold the OBJECT embed inside of it
     //(originally div)   newDiv = doc.createElement("cloze");
	newDiv = doc.createElement("span");
        // assign the given arguments and add space at the end
        for (var field in param) {
        //   var value = param[field];
      	      var value = param[field] + " ";
            if (!value) {
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
    ...
    switch (cmdID) {
        ...
        case "insertcloze": this._insertCloze(); break;
